<template>
  <el-dialog
    ...
    @close="clearData"
  >
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" size="small" label-width="100px" label-position="right">
      <el-form-item label="采集任务名称" prop="taskName">
        <el-input v-model="ruleForm.taskName" maxlength="20" placeholder="最大长度为20个字符" />
      </el-form-item>
      <el-row>
        <el-col :span="12">
          <el-form-item label="数据源名称" prop="datasourceId">
            <el-select v-if="isEditRelation.id===-1" v-model="ruleForm.datasourceId" placeholder="请选择数据源名称">
              <el-option v-for="item in dataOption" :key="item.id" :value="item.id" :label="item.datasourceName" />
            </el-select>
        ...
        <el-col :span="12">
          <el-form-item label="调度方式" prop="cycleType">
            <el-select v-model="ruleForm.cycleType" placeholder="请选择调度方式">
              <el-option value="0" label="采集一次" />
              <el-option value="1" label="多次采集" />
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item v-if="ruleForm.cycleType==='0'" label="采集频率" prop="triggerTime">
        <el-date-picker
      ...
      <el-form-item v-if="ruleForm.cycleType==='1'" label="采集频率" prop="jobCron">
        <el-col :span="6">
          <el-select v-model="ruleForm.jobCron" placeholder="请设置执行周期" @change="setCycle">
            <el-option v-show="false" label="请选择频率" value="4" />
            <el-option label="按月执行" value="0" />
            <el-option label="按周执行" value="1" />
            <el-option label="按天执行" value="2" />
            <el-option label="按小时执行" value="3" />
          </el-select>
        </el-col>
        <el-col v-if="CycleSelected==='0'" :span="17" class="year ml5">
          <el-col :span="8" class="mr5">
            <el-input-number v-model="ruleForm.exeDate1.value1" :min="1" :max="31" label="设置执行日" @change="handleChange" />
          </el-col>
          <el-col :span="13">
            <el-time-picker
        ...
        <el-col v-if="CycleSelected==='1'" :span="16" class="year ml5">
          <el-col :span="8">
        ...
        <el-col v-if="CycleSelected==='2'" :span="16" class="year ml5">
          <el-col :span="6">
            <el-time-picker
        ...
        <el-col v-if="CycleSelected==='3'" :span="16" class="year ml5">
          <el-select v-model="ruleForm.exeDate4.value1" size="small" multiple placeholder="请选择时间（可多选）">
            <el-option
