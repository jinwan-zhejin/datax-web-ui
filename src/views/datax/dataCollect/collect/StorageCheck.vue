  ...
  <section class="storage-check">
    ...
    <ul class="storage-check-list">
      <li v-for="item in sourceList" :key="item.id">
        ...
        <div class="check-set">
          <span @click="checkTask(item.datasourceId)"><svg-icon icon-class="审核" /></span>
        </div>
      </li>
    </ul>
    <el-pagination
    ...
    >
      ...
      >
        ...
        <el-form ref="InnerForm" :model="InnerForm" size="small" :rules="rules" label-width="70px" label-position="right">
          ...
          <p style="font-size: 12px;padding-left: 15px;"><i class="el-icon-warning" /> {{ InnerForm.warning }}</p>
        </el-form>
        <span slot="footer" class="dialog-footer">
          <el-button type="info" size="small" @click="closeInnerForm">取 消</el-button>
          <el-button type="danger" size="small" :loading="buttonLoading" @click="submitMetaForm('InnerForm')">确 定</el-button>
        </span>
      </el-dialog>
      <el-table
        ...
        >
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" content="审核" placement="top">
              <i class="table-icon-menu" @click="checkMetaData(scope.row.id)"><svg-icon icon-class="审核" /></i>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
