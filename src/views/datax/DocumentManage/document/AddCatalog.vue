<template>
    <el-dialog
        ...
        append-to-body>
        <el-form :model="addCataLogForm" :rules="rules" ref="addCataLogForm" size="small"  label-width="80px" label-position="right">
            <el-form-item label="目录名称" prop="name">
                <el-input v-model="addCataLogForm.name" maxlength="20" placeholder="最大长度为20个字符"></el-input>
            </el-form-item>
        ...
        <span slot="footer" class="dialog-footer">
                ...
                <el-button type="danger" size="small" @click="submitCatalog('addCataLogForm')">确 定</el-button>
            </span>
    </el-dialog>
</template>
...
<script>
    export default {
        ...
        methods: {
            submitCatalog(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$urlApi.dataStandard.addDocumentContent(this.addCataLogForm).then((res)=>{
                            this.$message({
                                message:'操作成功',
                                type:'success'
                            });
        ...
        watch:{
            isEditCatalog:{//深度监听，可监听到对象、数组的变化
                handler(val, oldVal){
                    if(val.id===undefined){
