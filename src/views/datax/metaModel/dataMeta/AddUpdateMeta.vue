<template>
    <el-dialog
        ...
        width="760px">
        <span slot="title" v-if="isEditMeta.id === ''">新增数据元</span>
        <span slot="title" v-if="!(isEditMeta.id === '')&& !isEditMeta.isHistory">编辑数据元</span>
        <span slot="title" v-if="isEditMeta.isHistory">数据元详情</span>
            ...
            <el-row>
                <el-col :span="12">
                    <el-form-item label="数据元名称" prop="dataElementName">
                        <el-input v-model="baseForm.dataElementName" :disabled="isEditMeta.isHistory" maxlength="20" placeholder="最大长度为20个字符"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="英文名称" prop="dataElementCode">
                        <el-input v-model="baseForm.dataElementCode" :disabled="isEditMeta.isHistory || !(isEditMeta.id === '')" maxlength="20" placeholder="输入字母数字和下划线"
                                  @blur="checkCode()"
         ...
         <!--   <el-form-item label="数据源标记">
            ...
            <el-form-item label="数据元类型" prop="dataElementType">
                ...
                <el-col :span="11" :offset="2">
                    <el-input v-model="baseForm.dataElementAttr" :placeholder="item.des" v-if="item.value===baseForm.dataElementType" :disabled="item.type===1 || isEditMeta.isHistory || !(isEditMeta.id === '') " :key="item.value" v-for="item in dataType" ></el-input>
                </el-col>
            </el-form-item>
            ...
            <el-row>
                <el-col :span="12">
                    <el-form-item
                        label="数据范围"
                        prop="dataRangeFront">
                        <el-input v-model.number="baseForm.dataRangeFront" placeholder="请输入较小数字" :disabled="isEditMeta.isHistory"></el-input>
                    </el-form-item>
                </el-col>
                ...
                <el-col :span="10">
                    <el-form-item
                        ...
                        prop="dataRangeEnd"
                    >
                        <el-input v-model.number="baseForm.dataRangeEnd" placeholder="请输入较大数字" :disabled="isEditMeta.isHistory"></el-input>
                    </el-form-item>
                </el-col>
            </el-row>
           ...
           <!-- </el-form-item>-->
            <el-form-item label="正则表达式">
                <el-select clearable v-model="baseForm.regularExpression"  size="small" placeholder="请选择数据源类型" :disabled="isEditMeta.isHistory">
                    <el-option value="^([1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3})|([1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X))$" label="身份证号码验证"></el-option>
                    <el-option value="^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$" label="手机号码验证"></el-option>
                    <el-option value="((25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))" label="IP地址验证"></el-option>
                    <el-option value="^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$" label="邮箱地址验证"></el-option>
                </el-select>
            </el-form-item>
