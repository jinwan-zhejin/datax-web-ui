<template>
    <section class="data-meta-section main-outer">
        ...
        <el-row class="main-inner">
            <el-row>
                <el-form :model="dataMetaForm" size="small" ref="dataMetaForm" class="demo-form-inline" label-width="80px">
                    <div class="content">
                        <el-row>
                            <el-col :span="8">
                                <el-form-item
                        ...
                        <el-col :span="24">
                            <el-button size="mini" @click="addNewMeta" class="grey-btn">新建</el-button>
                            <el-button size="mini" class="grey-btn" @click="sendAudit" :disabled="this.deleteSelection.length<=0">送审</el-button>
                            <el-button size="mini" @click="deleteList('s',1)" :disabled="this.deleteSelection.length<=0" class="red-btn">删除</el-button>
                            <el-input v-model="dataMetaForm.name" @keyup.enter.native="getData" placeholder="请输入数据元名称" style="width: 330px" size="mini">
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <el-tooltip  v-if="scope.row.status===0 || scope.row.status===3 || scope.row.status===4" class="item" effect="dark" content="编辑" placement="top">
                                <i class="table-icon-menu" @click="editData(scope.row.id)"><svg-icon iconClass="编辑1"></svg-icon></i>
                            </el-tooltip>
                            ...
                            <el-tooltip class="item" effect="dark" content="关联关系" placement="top">
                                <i class="table-icon-menu" @click="conbinRelation(scope.row.id)"><svg-icon iconClass="g-流程"></svg-icon></i>
                            </el-tooltip>
                            <el-tooltip v-if="scope.row.status===1 || scope.row.status===2 || scope.row.status===3" class="item" effect="dark" content="查看历史" placement="top">
                                <i class="table-icon-menu" @click="showHistory(scope.row.id)"><svg-icon iconClass="g-历史"></svg-icon></i>
                            </el-tooltip>
        ...
        <set-combin-relation :SetCombinVisible="SetCombinVisible" :isEditCombin="isEditCombin" @SetCombinBack="SetCombinBack"></set-combin-relation>
        <history-detail :HistoryVisible="HistoryVisible" :historyList="historyList" @HistoryBack="HistoryBack"></history-detail>
    </section>
</template>
