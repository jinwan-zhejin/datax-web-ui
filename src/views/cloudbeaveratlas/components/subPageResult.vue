...
<div :key="timer">
            ...
            <el-table class="tableStyle" :data="tableData" :header-cell-style="{background:'#F8F8FA',color:'#333333',fontWeight:'bold'}">
                <el-table-column key="名称" label="名称" prop="attributes.name" min-width="100" :show-overflow-tooltip="true" sortable>
                  ...
                  row.status === 'DELETED'
                    ? 'tableItemLinkRed'
                    : 'tableItemLink'
                ]" @click="goToDetails(row)"><i class="el-icon-document" />&nbsp;{{
                row.attributes.name
              }}</a>
                    ...
                    <template v-slot:default="{ row }">
                        <el-button-group v-if="row.classifications.length > 1" style="width: 150px;">
                            <el-tooltip :content="row.classifications[0].typeName">
                                <el-button plain size="mini" style="width:100px;overflow:hidden;text-overflow:ellipsis;" @click="
                      researchClassification(row.classifications[0].typeName)
                    ">{{ row.classifications[0].typeName }}</el-button>
                            </el-tooltip>
                            <el-button plain size="mini" style="width:12px;" icon="el-icon-close" @click="
    ...
    <el-dialog title="元数据比对" :visible.sync="metaCompareVisible" :before-close="initCompare" :show-close="false">
        <el-form ref="compareParams" :model="compareParams" label-position="left">
            ...
            <el-form-item v-if="compareType === 'time'" label="基线时间: " prop="baselineTime" :rules="{
            required: true,
            message: '请选择比较基线时间点',
            trigger: 'blur'
          }">
            ...
            <el-form-item v-if="compareType === 'time'" label="待比较时间点: " prop="toTime" :rules="{
            required: true,
            message: '请选择待比较时间点',
            trigger: 'blur'
          }">
                <el-select v-model="compareParams.toTime" placeholder="请选择待比较时间点" clearable>
                    <el-option v-for="(item, index) in versionListLite(
                compareParams.baselineTime
              )" :key="index" :label="formatDate(item.timestamp)" :value="item.timestamp" />
                </el-select>
            </el-form-item>
        </el-form>
