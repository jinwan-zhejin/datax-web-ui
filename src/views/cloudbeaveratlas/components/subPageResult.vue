...
<template>
  <div :key="timer">
      ...
      <el-col>
        ...
        <el-tooltip content="保存为自定义筛选器" placement="top">
          <el-button
            ...
            size="mini"
            plain
            @click="saveAsCustomFilter"
          ><svg-icon icon-class="save-light" /> 保存过滤器</el-button>
        </el-tooltip>
          ...
          >
            ...
            <span class="group">
              <span class="capsuleView">
                ...
                <i
                  class="el-icon-error"
                  @click="deleteFilterItem(index, item.name)"
                />
              </span>
            </span>
            <span>)</span>
      ...
      <el-col :span="24">
        <el-table class="tableStyle" :data="tableData" :header-cell-style="{background:'#F8F8FA',color:'#333333',fontWeight:'bold'}">
          ...
          <el-table-column key="分类" label="分类" width="150">
            <template v-slot:default="{ row }">
              <el-button-group
                v-if="row.classifications.length > 1"
                style="width: 150px;"
              >
              ...
              >
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item>
                    <div
                      v-for="(classes, index) in row.classifications"
                      :key="index"
                    >
                      <el-button-group v-if="index !== 0" style="width: 150px">
                        <el-tooltip :content="classes.typeName">
                          <el-button
                            ...
                            size="mini"
                            style="width:100px;overflow:hidden;text-overflow:ellipsis;"
                            @click="researchClassification(classes.typeName)"
                          >{{ classes.typeName }}</el-button>
                        </el-tooltip>
                        <el-button
                          plain
                          size="mini"
          ...
          <el-table-column key="操作" label="操作" min-width="115">
            <template v-slot:default="{ row }">
              <el-button
                ...
                plain
                size="mini"
                @click="metaCompare(row.guid)"
              >元数据比对</el-button>
            </template>
          </el-table-column>
          <el-table-column
            v-for="(item, index) in tableColumnsSelected"
            :key="index"
            :label="translateIt(item)"
            min-width="115"
          >
            <template v-slot:default="{ row }">
              ...
              <a
                v-if="item === 'instance'"
                class="tableItemLink"
                @click="goToDetails(instanceInfo(row))"
              >{{
    ...
    <el-dialog
      title="元数据比对"
      :visible.sync="metaCompareVisible"
      :before-close="initCompare"
      :show-close="false"
    >
      <el-form ref="compareParams" :model="compareParams" label-position="left">
        ...
        >
          <el-select
            v-model="compareParams.baselineTime"
            placeholder="请选择比较基线时间点"
            clearable
          >
            <el-option
              v-for="(item, index) in versionListLite(compareParams.toTime)"
              :key="index"
        ...
        >
          <el-select
            v-model="compareParams.toTime"
            placeholder="请选择待比较时间点"
            clearable
          >
            <el-option
    ...
    <AddCustomFilter
      :add-custom-filter-show="addCustomFilterShow"
      @closeaddcustomfilter="addCustomFilterShow = false"
    />
  </div>
</template>
  ...
  methods: {
    ...
    async getCompareResult() {
      ...
      this.$notify({
        title: '成功',
        message:
          "元数据比对任务提交成功, 请到'元数据比对'模块查看比对进度及结果详情！",
        type: 'success',
        duration: 3000
      });
      // }
