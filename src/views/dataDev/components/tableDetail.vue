...
export default {
  ...
  methods: {
    ...
    async initData(node) {
      ...
      const host = node ? node.parent.parent.data.jdbcUrl.split('://')[1].split('/')[0].split(':')[0] : ''
      const port = node ? node.parent.parent.data.jdbcUrl.split('://')[1].split('/')[0].split(':')[1] : ''
      const databaseName = node ? node.data.schema : ''
      ...
      switch (node ? node.parent.parent.data.datasource.toLowerCase() : '') {
        case 'mysql':
