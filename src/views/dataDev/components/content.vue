...
<el-container>
    <el-aside width="25%" class="contentLeft">
        <div class="dataBase">
            <p class="P-tit">数据连接</p>
            <el-select v-model="dataBaseid" style="width:100%" clearable placeholder="请选择数据库">
                <el-option v-for="item in dataBaseList" :key="item.id" :label="item.datasourceName" :value="item.id" />
            </el-select>
            <p class="P-tit">模式</p>
            <el-select v-model="schemaId" style="width:100%;" clearable placeholder="请选择Schema" @change="getTableList">
                <el-option v-for="item in SchemaList" :key="item" :label="item" :value="item" />
            </el-select>
        </div>
        <div class="table">
