...
<div class="app-container">
    <div class="filter-container">
        ...
        <el-card style="height: 660px; margin-top: 10px">
            <el-form ref="form" :model="form" label-width="200px" style="margin-top: 15px">
                <el-form-item label="所属项目名称">
                    <el-select style="width: 85%" v-model="form.projectId" placeholder="请选择待项目名称" @change="onProjectChange">
                        <el-option v-for="item in projectlist" :key="item.id" :value="item.id" :label="item.name" />
                    </el-select>
                </el-form-item>
                <el-form-item label="待转换数据源">
                    <el-select style="width: 85%" v-model="form.datasource" placeholder="请选择待转换数据源" @change="onDSChange">
                        <el-option v-for="item in datasourcelist" :key="item.id" :value="item.id" :label="item.datasourceName" />
                    </el-select>
                </el-form-item>
                <el-form-item label="待转换数据库/Schema">
                    <el-select style="width: 85%" v-model="form.schema" placeholder="请选择待转换数据库/Schema" @change="onSchemaChange">
                ...
                <el-form-item label="目标SQL类型">
                    <el-radio-group v-model="form.targetSQL">
                        <el-radio label="Hive"></el-radio>
                        <el-radio label="Impala"></el-radio>
                        <el-radio label="Kudu"></el-radio>
                    </el-radio-group>
                </el-form-item>

                <el-form-item>
                    <el-button style="margin-left: 30%" type="primary" size="small" @click="onTransform">开始转换</el-button>
                    <el-button type="primary" size="small" @click="showTransformedSQL">查看转换结果</el-button>
                </el-form-item>
                <el-progress v-if="showProgressbar" style="width: 80%; margin-left: 10%" ref="transformProgress" :percentage="transformPercentage"></el-progress>
            </el-form>
        </el-card>

        <el-dialog title="查看转换结果" :visible.sync="dialogVisible" width="60%">
            <textarea v-model="sqlScript" class="codesql" style="width: 100%; height: 550px" />
        </el-dialog>
    </div>
            ...
            if (targetSQL == 'Hive') {
                ...
                for (let k = 0; k < len; k++) {
                    await getTableColumns({
                        ...
                        const arr = []
                        var script = 'CREATE TABLE IF NOT EXISTS ' + this.tablelist[k].name + ' (\n'
                        for (let j = 0; j < res.datas.length; j++) {
                            var ele = res.datas[j]
                            script += ('  ' + ele.COLUMN_NAME + ' ' + this.mysql2hiveDataTypeMap[ele.DATA_TYPE] + ' comment \'' + ele.COLUMN_COMMENT + '\',\n')
                        }
